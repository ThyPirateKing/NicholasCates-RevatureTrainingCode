...
SELECT FirstName, LastName, CustomerId, Country FROM [MyDatabase].[dbo].[Customer]
        WHERE Country != 'USA';
...
SELECT BillingCountry FROM [MyDatabase].[dbo].[Invoice];
...
SELECT COUNT(*) FROM [MyDatabase].[dbo].[Invoice]
        WHERE InvoiceId = 37;

-- how many invoices per country? BillingCountry  # of invoices -
SELECT COUNT(InvoiceLine) FROM [MyDatabase].[dbo].[Customer];
...
SELECT Track.Name AS 'Track Name', Genre.Name AS 'Genre Name', Artist.Name AS 'Artist Name'
...
-- Retrieve the total sales per country, ordered by the highest total sales first.
SELECT BillingCountry , SUM(Total) AS Country_Sum
        FROM [MyDatabase].dbo.[Invoice] 
